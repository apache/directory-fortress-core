 ...
 * activation within a particular user's session.  This method is called from {@link us.jts.fortress.util.time.CUtil#validateConstraints} during createSession
    ...
    implements Validator
{
    ...
    /**
     * This method is called during entity activation, {@link us.jts.fortress.util.time.CUtil#validateConstraints} and ensures the role does not violate dynamic separation of duty constraints.
     *
     * @param session    contains list of RBAC roles {@link us.jts.fortress.rbac.UserRole} targeted for activation.
     * @param constraint required for Validator interface, not used here..
     * @param time       required for Validator interface, not used here.
     * @return '0' if validation succeeds else {@link us.jts.fortress.GlobalErrIds#ACTV_FAILED_DSD} if failed.
     */
    @Override
                    ...
                    while (activatedRoles.hasNext())
                    {
                        ...
                        if (map.contains(activatedRole.getName()))
                        {
                            ...
                            if (matchCount >= dsd.getCardinality())
                            {
                                ...
                                String warning = CLS_NM + ".validate userId [" + session.getUserId() + "] failed activation of assignedRole [" + activatedRole.getName() + "] validates DSD Set Name:" + dsd.getName() + " Cardinality:" + dsd.getCardinality();
                                log.warn(warning);
                            ...
                            for (String parentRole : parentSet)
                            {
                                if (map.contains(parentRole))
                                {
                                    ...
                                    if (matchCount >= dsd.getCardinality())
                                    {
                                        ...
                                        activatedRoles.remove();
                                        String warning = CLS_NM + ".validate userId [" + session.getUserId() + "] assignedRole [" + activatedRole.getName() + "] parentRole [" + parentRole + "] validates DSD Set Name:" + dsd.getName() + " Cardinality:" + dsd.getCardinality();
                                        log.warn(warning);
                                        rc = GlobalErrIds.ACTV_FAILED_DSD;
                                    }
